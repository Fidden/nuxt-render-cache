...
const response = await $fetch('/api/render-cache/keys/page:home', {
  headers: { 'x-render-cache-api': token },
});

if (response.exists) {
  console.log('Данные найдены:', response.data);
  console.log('Теги:', response.tags);
  console.log('Время создания:', new Date(response.timestamp));
} else {
...
curl -H "x-render-cache-api: token" \
     http://localhost:3000/api/render-cache/keys/component:header

# Получение размера кешированных данных
curl -H "x-render-cache-api: token" \
     http://localhost:3000/api/render-cache/keys/page:products | jq '.data | length'
```
...
if (response.deleted) {
  ...
  console.log('Ключ не найден или уже был удален');
}
```

```bash
# Очистка кеша для конкретной страницы
curl -X DELETE \
     -H "x-render-cache-api: token" \
     http://localhost:3000/api/render-cache/keys/page:about

# Удаление кеша компонента
curl -X DELETE \
     -H "x-render-cache-api: token" \
     http://localhost:3000/api/render-cache/keys/component:sidebar
```
...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    if (health.redis.memory.used > 500 * 1024 * 1024) {
      ...
      alerts.push({
        level: 'critical',
        message: `High memory usage: ${(health.redis.memory.used / 1024 / 1024).toFixed(2)} MB`,
      });
    }
    ...
    setInterval(
      async () => {
        try {
          ...
          if (cleaned > 0) {
            console.log(
              `Scheduled cleanup: removed ${cleaned} expired entries`
            );
          }
        } catch (error) {
